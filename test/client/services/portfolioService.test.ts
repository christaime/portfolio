...
describe('portfolioService', () => {
  ...
  it('should switch dataset when setLanguage is invoked', async () => {
    portfolioService.setLanguage('fr');
    const engineerFr = await portfolioService.getEngineerInfo();
    expect(engineerFr.title).toContain('Ingénieure');

    portfolioService.setLanguage('en');
    const engineerEn = await portfolioService.getEngineerInfo();
    expect(engineerEn.title).toContain('Engineer');
  });
});
